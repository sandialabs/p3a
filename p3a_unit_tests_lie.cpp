...
TEST(tensor, exp)
{
  ...
  auto const a          = p3a::sqrt(2.0) / 2.0;
  auto const b          = p3a::sqrt(3.0) / 3.0;
  auto const c          = p3a::sqrt(6.0) / 6.0;
  auto const V          = Tensor(c, a, b, c, -a, b, -2 * c, 0, b);
  auto const p          = p3a::exp(1.0);
  auto const q          = p3a::exp(2.0);
  auto const r          = p3a::exp(4.0);
